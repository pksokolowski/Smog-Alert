...
object CreateTriggersCallback: RoomDatabase.Callback(){
    override fun onOpen(db: SupportSQLiteDatabase) {
        super.onOpen(db)
        db.execSQL("CREATE TRIGGER IF NOT EXISTS delete_old_aq_logs AFTER INSERT ON air_quality_logs BEGIN DELETE FROM air_quality_logs WHERE id NOT IN (SELECT id FROM air_quality_logs ORDER BY id DESC LIMIT 2); END")
